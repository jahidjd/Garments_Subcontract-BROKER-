   ...
   public function deal_collection_ledger($id)
{
   ...
   public function timeline(){
      ...
      return view('front.timeline',compact('machine_order','work_order','a','proposal'));
   }
...
// jahid end 
      ...
      //   dd($order);
        ...
        public function workorder_details($id){
         ...
         if(Auth::user()->name){
            ...
            //   dd($work_order);
         return view('front.workorder_details',compact('w_details','data','mahinepost'));
   ...
   public function deal_list(){
      ...
      $deal_list=Deals::where('merchant_id',$companyID)->orwhere('seller_id',$companyID)->orderBy('id','DESC')->get();
   //   dd($deal_list);
     $request = '';
     if(isset($deal_list)){
  ...
  public function deal_details($id){
   ...
   function proposalFrom($id,$name){
      ...
      if($name=='workOrder'){
         ...
         $company = Auth::user()->company_id;
         $workOrder = Workorders::where('company_id',$company)->get();
   ...
   function savePropusal(request $data){
      ...
      if(isset($proposal)){
         foreach($proposal as $p){
            if($data->workorder=='workorderPost'){
               if($p->machinepost_id == $data->machinepost_id){
                  return redirect(route('timeline'));
               }
            }
            if($data->machinepost=='machinePost'){
               if($p->workorder_id == $data->workorder_id ){
                  return redirect(route('timeline'));
               }
            }
   ...
   if($proposalDetails->user->company_id == Auth::user()->company_id){
      $status = "proposal i send";
      
     
   }else{
      $status = "proposal i recive";
     
   }
   //  dd($status);
    return view('front.proposal_details',compact('data','proposalDetails','status'));
   }
...
//end shimo
      ...
      function machineProposalInd($id){
         $proposal = Proposal::where('machinepost_id',$id)->get();
         $data = User::find(Auth::user()->id);
         return view('front.machineProposalInd',compact('data','proposal'));

      }
      function workorderPostInd($id){
         $proposal = Proposal::where('workorder_id',$id)->get();
         $data = User::find(Auth::user()->id);
         return view('front.workorderPostInd',compact('data','proposal'));
      }
      function acceptProposal($id){
         $proposal = Proposal::find($id);
         $array =[
            'merchant_id'=>$proposal->workOrder->company_id,
            'seller_id'=>$proposal->machinePost->userMachine->company_id,
            'machine_id'=>$proposal->machinePost->userMachine->id,
            'title'=>$proposal->title,
            'specifications'=>$proposal->machinePost->userMachine->specifications,
            'budget'=>$proposal->budget,
            'deadline'=>$proposal->deadline,
            'quantity'=>$proposal->quantity,
            'quality_related'=>$proposal->quality_related,
         ];
         $deals = Deals::create($array);
         if($deals){
            return redirect(route('deal_list'));
         }
