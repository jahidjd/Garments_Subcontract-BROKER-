  ...
  <div class="container"> 
    <div class="row"> 
      ...
      <div class="col-md-9 col-sm-7">
        ...
        @if(isset($work_order) || isset($machine_order) )
        @if(isset($work_order))
        @foreach($work_order as $l)
        @if($l->company_id != Auth::user()->company_id)
        <div class="job-verticle-list">
          <div class="vertical-job-card">
            <div class="vertical-job-header">
              <div class="vrt-job-cmp-logo"> <a href="{{route('view_timeline',$l->id)}}"><img src="{{asset('photos/company/'.$l->Companies->photo)}}" class="img-responsive" alt="" /></a> </div>
              <h4><a href="{{route('view_timeline',$l->id)}}">{{$l->title}}</a></h4>
              <span class="com-tagline">2</span> <span class="pull-right vacancy-no">No. <span class="v-count">2</span></span> 
		        	</div>
            <div class="vertical-job-body">
              <div class="row">
                <div class="col-md-9 col-sm-12 col-xs-12">
                  <ul class="can-skils">
                    <li><strong>Company Name :</strong>{{$l->companies->name}}</li>
                    
                    
                    <li><strong>Category : </strong>{{$l->category->name}}</li>
                    <li><strong>Specifications </strong> <div><span class="skill-tag"></span> <span class="skill-tag">spinnig:2</span> <span class="skill-tag">speed:13rpm</span> <span class="skill-tag">Brand:Maruto</span></div> </li>
                    <li><strong>Budget : </strong>{{$l->budget}}</li>
                    <li><strong>Deadline : </strong>{{  date("d M Y", strtotime($l->deadline)) }}</li>
                  </ul>
                </div>
                <div class="col-md-3 col-sm-12 col-xs-12">
                  <div class="vrt-job-act"> <a href="{{route('view_timeline',$l->id)}}"  class="btn-job theme-btn job-apply">View Post</a> 
                    
                    {{-- @if (isset($proposal))
                        @foreach ($proposal as $p)
                            @if($p->machinePost->userMachine->company_id == $l->company_id)
                            <a href="" title="" class="btn-job light-gray-btn"  onclick="return confirm('Proposal submitted')">Submitted</a> 
                            @endif
                        @endforeach
                    @endif --}}
                      <a href="{{route('proposalFrom',['id'=>$l->id,'name'=>'workOrder'])}}" title="" class="btn-job light-gray-btn">Submit Proposal</a> </div>
      ...
      @if(isset($machine_order))
        @foreach($machine_order as $l)
        @if($l->userMachine->company_id != Auth::user()->company_id)
        <div class="job-verticle-list">
          <div class="vertical-job-card">
            <div class="vertical-job-header">
              <div class="vrt-job-cmp-logo"> <a href="{{route('view_timeline',$l->id)}}"><img src="{{asset('photos/company/'.$l->usermachines->company->photo)}}" class="img-responsive" alt="" /></a> </div>
              <h4><a href="job-detail.html">{{$l->title}}</a></h4>
              <span class="com-tagline">2</span> <span class="pull-right vacancy-no">No. <span class="v-count">2</span></span> 
		        	</div>
               <div class="vertical-job-body">
              <div class="row">
                <div class="col-md-9 col-sm-12 col-xs-12">
                  <ul class="can-skils">
                    <li><strong>Company Name :</strong>{{$l->usermachines->company->name}}</li>
                    
                    
                    <li><strong>Category : </strong>{{$l->usermachines->category->name}}</li>
                    <li><strong>Specifications </strong> <div><span class="skill-tag"></span> <span class="skill-tag">spinnig:2</span> <span class="skill-tag">speed:13rpm</span> <span class="skill-tag">Brand:Maruto</span></div> </li>
                    <li><strong>Available Machines: </strong>{{$l->number_of_machine}}</li>
                    <li><strong>Deadline : </strong>{{  date("d M Y", strtotime($l->deadline)) }}</li>
                  </ul>
                </div>
                <div class="col-md-3 col-sm-12 col-xs-12">
                  <div class="vrt-job-act"> <a href="{{route('go_timeline',$l->id)}}" class="btn-job theme-btn job-apply">View Post</a> 
                    {{-- @if (isset($proposal))
                        @foreach ($proposal as $p)
                            @if($p->workOrder->company_id ==$l->userMachine->company_id)
                            <a href="" title="" class="btn-job light-gray-btn"  onclick="return confirm('Proposal submitted')">Submitted</a>
                            @endif
                        @endforeach
                    @endif --}}
                    
                      <a href="{{route('proposalFrom',['id'=>$l->id,'name'=>'machine'])}}" title="" class="btn-job light-gray-btn">Submit Proposal</a> </div>
